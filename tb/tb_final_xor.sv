...
module tb_final_xor;
  ...
  real x_samp[NUM_SAMPLES][2]; // raw inputs
  ...
  localparam int  LEARN_TICKS_PER_SAMPLE = 80;
  ...
  // -------------------------
  // Build dataset: y = A_gt * tanh(B_gt * x + b1_gt) + b2_gt
  // -------------------------
  task automatic build_dataset();
    real t, x1, x2;
    real h0, h1;

    // Teacher weights (keep moderate to avoid tanh saturation)
    B_gt[0][0] =  1.0;  B_gt[0][1] = -0.6;
    B_gt[1][0] =  0.5;  B_gt[1][1] =  0.9;
    ...
    for (int s = 0; s < NUM_SAMPLES; s++) begin
      // deterministic sweep over a diagonal line in R^2
      ...
      // Hidden with bias
      h0 = $tanh(B_gt[0][0]*x1 + B_gt[0][1]*x2 + b1_gt[0]);
      h1 = $tanh(B_gt[1][0]*x1 + B_gt[1][1]*x2 + b1_gt[1]);

      // Output with bias
      y_samp[s] = A_gt[0]*h0 + A_gt[1]*h1 + b2_gt;
    end
  endtask
  ...
  task automatic clamp_train(input real x1, input real x2, input real y);
    // top clamped to RAW [x1, x2] (top is LINEAR so this is direct)
  ...
  initial begin
    ...
    $display("[TB] Begin training (2->2->1, top LINEAR + hidden TANH)");
    $display("[TB] Teacher: y = A_gt * tanh(B_gt * x + b1_gt) + b2_gt");
    $display("[TB] A_gt = [%0.4f, %0.4f], b2_gt = %0.4f", A_gt[0], A_gt[1], b2_gt);
    $display("[TB] b1_gt = [%0.4f, %0.4f]", b1_gt[0], b1_gt[1]);
    ...
    for (int ep = 0; ep < EPOCHS; ep++) begin
      for (int s = 0; s < NUM_SAMPLES; s++) begin
        ...
        set_rates_infer();
        for (int tt = 0; tt < INFER_TICKS_PER_SAMPLE; tt++) do_tick();

        set_rates_learn();
        for (int tt = 0; tt < LEARN_TICKS_PER_SAMPLE; tt++) do_tick();
      end
