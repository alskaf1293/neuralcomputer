typedef enum logic [1:0] {
  ACT_LINEAR  = 2'd0,
  ACT_RELU    = 2'd1,
  ACT_SIGMOID = 2'd2   // placeholder: currently behaves as linear
} act_kind_e;

// Generic scalar activation wrapper in recFN space.
// - KIND = ACT_LINEAR  => f(x) = x,     f'(x) = 1
// - KIND = ACT_RELU    => f(x) = ReLU,  f'(x) = ReLU'
// - KIND = ACT_SIGMOID => currently same as linear (TODO: replace with real sigmoid)
module activation32 #(
    ...
    generate
      if (KIND == ACT_LINEAR) begin : G_LINEAR
        ...
        // Placeholder: acts as linear until you add a real sigmoid module.
        // To implement true sigmoid, replace this block with a proper
        // activation_sigmoid32 instance and its derivative.
        assign f_rec  = in_rec;
        assign fp_rec = rec_one;
...
module neuron_core_single_back #(
    ...
    parameter logic [31:0] BIAS_LR_SCALE_IEEE = 32'h3DCCCCCD, // ~0.1
    parameter bit          FREEZE_BIAS        = 1'b1,
    ...
    // ------------- neuron state and effective clamp ---------
    ...
    logic [RECW-1:0] xj_act, xj_actp_unused;
    wire             is_bias_idx = (j == N);
    wire [RECW-1:0]  xj_in       = is_bias_idx ? rec_one : x_col_rec[(j < N) ? j : 0];
    ...
    // φ(x_i_eff) and φ'(x_i_eff) for state dynamics
    logic [RECW-1:0] phi_xi, phi_prime_xi;
...
module pc_layer #(
  ...
  for (genvar gx=0; gx<K; gx++) begin
    assign x_state_ieee[gx] = x_set_en_vec[gx] ? x_obs_ieee_vec[gx] : x_i_ieee[gx];
  end
