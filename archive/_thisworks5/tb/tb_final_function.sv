...
module tb_pc3_hidden_relu_net_mse;
  ...
  localparam int LEARN_TICKS_PER_SAMPLE = 1;     // like MATLAB
  localparam int EVAL_SETTLE_TICKS      = 50;
  localparam int EPOCHS                = 100;
  ...
  function automatic real relu_r(input real z);
    ...
    // ---------- grid config ----------
    // NUM_SAMPLES must equal NX*NY

    localparam real X0_MIN = -1.2;
    localparam real X0_MAX =  1.2;

    // Pick a reasonable range for x1 (old param path was ~[-1.1, +1.1])
    localparam real X1_MIN = -1.1;
    localparam real X1_MAX =  1.1;
    ...
    if (NUM_SAMPLES != (NX*NY)) begin
      $fatal("[TB] NUM_SAMPLES (%0d) must equal NX*NY (%0d*%0d=%0d)",
             NUM_SAMPLES, NX, NY, NX*NY);
    end
    ...
    // ---------- grid samples ----------
    // map s -> (ix, iy), row-major
    // ix in [0..NX-1] controls x0, iy in [0..NY-1] controls x1
    for (s = 0; s < NUM_SAMPLES; s++) begin
      ix = s % NX;
      iy = s / NX;

      // linear sweep in each axis
