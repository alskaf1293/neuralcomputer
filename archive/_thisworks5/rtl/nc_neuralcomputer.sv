typedef enum logic [2:0] {
  ...
  ACT_TANH   = 3'd2,
  ACT_LRELU  = 3'd3
} act_kind_e; // NEED TO: implement sigmoid
...
module activation32 #(
    ...
    generate
      if (KIND == ACT_LINEAR) begin : G_LINEAR
        ...
        hf_rec2f32 U_R2F_IN (.in_rec(in_rec), .out_ieee(in_ieee_bits)); // <-- REQUIRED

      `ifndef SYNTHESIS
        real xin_r, y_r, dy_r;
        int unsigned out_u, outp_u;

        always_comb begin
          xin_r = f32_to_real(in_ieee_bits);
          y_r  = $tanh(xin_r);
          dy_r = 1.0 - (y_r*y_r);
          out_u  = real_to_f32(y_r);
          outp_u = real_to_f32(dy_r);
          f_ieee_bits  = out_u[31:0];
          fp_ieee_bits = outp_u[31:0];
        end
      `else
        always_comb begin
      ...
      end else if (KIND == ACT_LRELU) begin : G_LRELU
      logic [RECW-1:0] f_lrelu, fp_lrelu;
        activation_lrelu32 #(
          .EXP(EXP), .SIG(SIG),
          .LEAK_IEEE(32'h3A83126F) // 1e-3; tweak if you want
        ) U_ACT_LRELU (
          .in_rec(in_rec),
          .f_rec(f_lrelu),
          .fp_rec(fp_lrelu)
        );
        assign f_rec  = f_lrelu;
        assign fp_rec = fp_lrelu;
...
module neuron_core_single_back #(
    ...
    // x_vec_ieee = x^{(l+1)} coming from the layer above (presynaptic activities)
    ...
    logic [RECW-1:0] x_i_act;
    assign x_i_act = x_i_eff;

    // Compute f'(x_i) for the current activity (for activity dynamics gating)
    logic [RECW-1:0] fpi_act;

    activation32 #(
      .KIND(ACT_PRESYN_KIND),   // <-- use the SAME nonlinearity you mean by f(.) in the paper
      .EXP(EXP), .SIG(SIG)
    ) ACT_CURR_DERIV (
      .in_rec(x_i_act),         // derivative evaluated at current activity
      .f_rec(),                 // unused
      .fp_rec(fpi_act)
    );
