// ======================================================================
// tb_pc3_two_two_one_hidden_relu_net_mse.sv
//
// 3-layer pc_network_nlayer, K0=1 (bottom/output), K1=2 (hidden), K2=2 (top/input)
// Hidden layer uses ReLU, top and bottom are linear.
//
// Teacher:
//   h = ReLU(B_gt * x)
//   y = A_gt * h
...
module tb_pc3_relu_net_mse;
  ...
  // -------------------------
  // Build dataset: y = A_gt * ReLU(B_gt * x)
  // -------------------------
  task automatic build_dataset();
    real t, x1, x2;
    real h0, h1;
    // Pick something that uses both inputs and both hidden units
    ...
    for (int s = 0; s < NUM_SAMPLES; s++) begin
      ...
      // h = ReLU(B*x)
      h0 = relu_r(B_gt[0][0]*x1 + B_gt[0][1]*x2);
      h1 = relu_r(B_gt[1][0]*x1 + B_gt[1][1]*x2);
  ...
  task automatic clamp_train(input real x1, input real x2, input real y);
    ...
    // bottom clamped to y (neuron 0 used, neuron 1 padding)
  ...
  initial begin
    ...
    $display("[TB] Begin training (2->2->1, hidden ReLU)");
    $display("[TB] Teacher: y = A_gt * ReLU(B_gt * x)");
