// ======================================================================
// tb_pc3_relu_wx_mse_2to1to1.sv
//
// 3-layer pc_network_nlayer, ONE hidden unit:
//
//   top  : K2=2 (x1,x2)            [clamped to input]
//   hid  : K1=1 (ReLU state)       presyn features are LINEAR(x_j)
//   bot  : K0=1 (y)                [clamped during training]
//
// Teacher (ReLU AFTER first weight multiply):
//   y = a_gt * ReLU( w0_gt*x1 + w1_gt*x2 + b_gt )
//
// Notes:
// - This uses ACT_STATE on the hidden layer to apply ReLU to u = Wx (+b)
// - ACT_PRESYN is LINEAR everywhere (no ReLU on raw x_j)
// ======================================================================
...
module tb_pc3_relu_net_mse;
  ...
  // Teacher params: y = a*ReLU(w0*x1 + w1*x2 + b)
  ...
  //
  // Want ReLU AFTER the first weight multiply (hidden state):
  //   hidden: presyn φ = LINEAR (no rectification on raw x_j)
  //   hidden: state  φ = ReLU   (rectify u = Wx (+b))
  // =========================================================
  localparam act_kind_e ACT_PRESYN_LUT[NUM_LAYERS] = '{
    ACT_RELU,  // bottom
    ACT_LINEAR,  // hidden  <-- IMPORTANT
    ACT_LINEAR   // top
  };

  localparam act_kind_e ACT_STATE_LUT[NUM_LAYERS] = '{
    ACT_LINEAR,  // bottom (y linear)
    ACT_LINEAR,    // hidden  <-- ReLU(Wx)
    ACT_LINEAR   // top
  };
  ...
  // -------------------------
  // Build dataset: 2D grid, ReLU(Wx+b) teacher
  // -------------------------
  task automatic build_dataset();
    int G, ix, iy;
    real tx, ty, x1, x2;

    // Pick something that actually crosses 0 within [-1,1]^2 so ReLU matters
    w0_gt =  1.2;
    w1_gt = -0.7;
    b_gt  =  -0.6;
    a_gt  =  1.7;
    ...
    for (int s = 0; s < NUM_SAMPLES; s++) begin
      ...
      // y = a*ReLU(w0*x1 + w1*x2 + b)
      y_samp[s] = a_gt * relu_r(w0_gt*x1 + w1_gt*x2 + b_gt);
    end
  endtask
  ...
  initial begin
    ...
    $display("[TB] Begin training (2->1->1, ReLU on hidden state: ReLU(Wx+b))");
    $display("[TB] Teacher: y = %0.4f*ReLU(%0.4f*x1 + %0.4f*x2 + %0.4f)",
