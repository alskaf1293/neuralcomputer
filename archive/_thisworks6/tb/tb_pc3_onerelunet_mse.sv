// ======================================================================
// tb_pc3_relu_inputs_mse_2to1to1.sv
//
// 3-layer pc_network_nlayer, ONE hidden unit:
//
//   top  : K2=2 (x1,x2)         [clamped to input]
//   hid  : K1=1 (LINEAR state)   presyn features are ReLU(x_j)
//   bot  : K0=1 (y)             [clamped during training]
//
// Teacher (ReLU on raw inputs, NOT on weighted sum):
//   y = a_gt * ( w0_gt*ReLU(x1) + w1_gt*ReLU(x2) + b_gt )
//
// Notes:
// - This uses ACT_PRESYN on the hidden layer to apply ReLU to x1,x2
// - Hidden ACT_STATE is set to LINEAR so the hidden neuron is linear in u
// ======================================================================
...
module tb_pc3_relu_net_mse;
  ...
  // Teacher params: y = a*(w0*ReLU(x1) + w1*ReLU(x2) + b)
  real w0_gt, w1_gt, a_gt, b_gt;
  ...
  //
  // We want ReLU on the *raw* inputs feeding the hidden neuron.
  // That means: hidden layer presynaptic φ = ReLU.
  // But hidden state's own φ should be LINEAR (so no extra ReLU).
  // =========================================================
  localparam act_kind_e ACT_PRESYN_LUT[NUM_LAYERS] = '{
    ACT_LINEAR,   // bottom: presyn from hidden is linear
    ACT_RELU,     // hidden: presyn from top is ReLU(x_j)
    ACT_LINEAR    // top: unused presyn (no layer above), keep linear
  };

  localparam act_kind_e ACT_STATE_LUT[NUM_LAYERS] = '{
    ACT_LINEAR,   // bottom state
    ACT_LINEAR,   // hidden state MUST be linear for "ReLU on inputs only"
  ...
  // -------------------------
  // Build dataset: 2D grid, input-rectified teacher
  // -------------------------
  task automatic build_dataset();
    int G, ix, iy;
    real tx, ty, x1, x2;

    // Pick something that uses both inputs and a bias
    w0_gt =  1.2;
    w1_gt = -0.7;
    b_gt  =  0.0;
    a_gt  =  1.7;
    ...
    for (int s = 0; s < NUM_SAMPLES; s++) begin
      ...
      // y = a*(w0*ReLU(x1) + w1*ReLU(x2) + b)
      y_samp[s] = a_gt * ( w0_gt*relu_r(x1) + w1_gt*relu_r(x2) + b_gt );
    end
  endtask
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) start_q_tb <= 1'b0; else start_q_tb <= start_tick;
  end
  wire start_edge_tb = start_tick & ~start_q_tb;

  always_ff @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
      done_q_tb <= 1'b0; done_edge_flag <= 1'b0;
    end else begin
  ...
  task automatic init_hidden_only();
    ...
    alpha_ieee = f2b(0.0); gamma_ieee = f2b(GAMMA_R);
  ...
  initial begin
    ...
    $display("[TB] Begin training (2->1->1, ReLU on inputs only)");
    $display("[TB] Teacher: y = %0.4f*(%0.4f*ReLU(x1) + %0.4f*ReLU(x2) + %0.4f)",
             a_gt, w0_gt, w1_gt, b_gt);
