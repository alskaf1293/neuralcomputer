...
module tb_pc2_two_one_linear_net;
  ...
  task automatic build_dataset();
    int idx;
    int S = $rtoi($sqrt(NUM_SAMPLES));
    if (S*S != NUM_SAMPLES)
      $fatal("[TB] NUM_SAMPLES must be a perfect square for 2D grid.");

    idx = 0;
    for (int i = 0; i < S; i++) begin
      for (int j = 0; j < S; j++) begin
        // Uniform grid in [-1, 1] × [-1, 1]
        real x0 = -1.0 + 2.0 * real'(i) / real'(S-1);
        real x1 = -1.0 + 2.0 * real'(j) / real'(S-1);

        x0_samp[idx] = x0;
        x1_samp[idx] = x1;
        y_samp [idx] = a0_gt*x0 + a1_gt*x1 + b_gt;
        idx++;
      end
    end
  endtask
