...
The first experiment (Section~\ref{sec:exp-tanh})
is implemented as a Verilator simulation using the reference RTL implementation.
...
Biases are included as a fixed presynaptic lane in the hidden layer and
initialised from $\mathcal{N}(0,\,0.5^2)$.
...
(Figure~\ref{fig:scaling}).  The random bias initialisation raises the initial
MSE to $0.452$ for $2\rightarrow4\rightarrow3$, $0.380$ for
$4\rightarrow8\rightarrow4$, and $0.477$ for $8\rightarrow16\rightarrow8$.
Despite this elevated starting point, all three networks converge to small
residuals by epoch~25: $5.0\times10^{-4}$, $5.2\times10^{-4}$, and
$2.3\times10^{-4}$ respectively, with no divergence or oscillation at any
scale.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\linewidth]{figures/python_runs_scaling.png}
...
$y = A \cdot \tanh(B \cdot \tanh(x))$ and bias weights initialised from
$\mathcal{N}(0,\,0.5^2)$.  All three configurations converge cleanly
without oscillation.}
\label{fig:scaling}
\end{figure}
...
\hline
Scaling: small  & \(2 \rightarrow 4 \rightarrow 3\)  & 0.4519 & 0.000501 \\
Scaling: medium & \(4 \rightarrow 8 \rightarrow 4\)  & 0.3804 & 0.000516 \\
Scaling: large  & \(8 \rightarrow 16 \rightarrow 8\) & 0.4771 & 0.000231 \\
\hline
\end{tabular}
\caption{Final MSE values reported at epoch~25. The top row is the
reference regression experiment (RTL simulation); the bottom three are the
architectural scaling sweep (Python reference, sparse tiled tanh teacher,
bias weights initialised from $\mathcal{N}(0,\,0.5^2)$).}
\label{tab:results-summary}
\end{table}

Across all four runs, end-to-end supervised learning proceeds using only local
state variables, local prediction errors, and adjacent-layer communication
under a fixed FSM schedule. Nonzero residual floors in the RTL experiment
